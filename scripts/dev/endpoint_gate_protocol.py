...
def _measurement_errors(
    ...
    *,
    ...
    if not isinstance(median, Mapping):
        return [f"{prefix}.median: must be an object"]
    if not isinstance(runs, list):
        return [f"{prefix}.runs: must be a list"]

    errors = []
    expected_runs = int(protocol["runs"])
    measured_frames = int(protocol["measured_frames"])
    total_frames = int(protocol["total_frames"])
    if len(runs) != expected_runs:
        errors.append(f"{prefix}.runs.length: expected {expected_runs}, got {len(runs)}")
    ...
    def number_error(value: Any, field: str, *, allow_zero: bool) -> str | None:
        ...
        if numeric < 0 or (numeric == 0 and not allow_zero):
            ...
            return f"{field}: must be {relation}"
        return None

    for field in _MEDIAN_FIELDS:
        error = number_error(
            median.get(field, _MISSING),
            f"{prefix}.median.{field}",
            allow_zero=field in nonnegative,
        )
        if error:
            errors.append(error)

    for index, run in enumerate(runs):
        run_prefix = f"{prefix}.runs[{index}]"
        if not isinstance(run, Mapping):
            errors.append(f"{run_prefix}: must be an object")
            continue
        if run.get("run") != index + 1:
            errors.append(f"{run_prefix}.run: expected {index + 1}")
        for field in _MEDIAN_FIELDS:
            error = number_error(
                run.get(field, _MISSING),
                f"{run_prefix}.{field}",
                allow_zero=field in nonnegative,
            )
            if error:
                errors.append(error)
        for field in ("frames_in", "frames_written"):
            if run.get(field) != total_frames:
                errors.append(
                    f"{run_prefix}.{field}: expected {total_frames}, got {run.get(field)!r}"
                )
        for field in ("conditions_start", "conditions_end"):
            actual_conditions = run.get(field)
            if not isinstance(actual_conditions, Mapping):
                errors.append(f"{run_prefix}.{field}: must be an object")
                continue
            for condition in ("power_source", "low_power_mode", "thermal_state"):
                actual = actual_conditions.get(condition, _MISSING)
                expected = expected_conditions.get(condition, _MISSING)
                if actual != expected:
                    errors.append(
                        f"{run_prefix}.{field}.{condition}: expected "
                        f"{_display(expected)}, got {_display(actual)}"
                    )
        steady = run.get("steady_ms_per_frame")
        steady_total = run.get("steady_total_ms")
        if all(
            isinstance(value, (int, float))
            and not isinstance(value, bool)
            and math.isfinite(float(value))
            for value in (steady, steady_total)
        ) and not math.isclose(
            float(steady_total),
            float(steady) * measured_frames,
            rel_tol=1e-12,
            abs_tol=1e-9,
        ):
            errors.append(
                f"{run_prefix}.steady_total_ms: inconsistent with "
                "steady_ms_per_frame and measured_frames"
            )
        total_parts = [
            run.get(field)
            for field in (
        ...
        if all(
            ...
            and not isinstance(value, bool)
            and math.isfinite(float(value))
            for value in (*total_parts, total)
        ) and not math.isclose(
            float(total),
            sum(float(value) for value in total_parts),
            rel_tol=1e-12,
            abs_tol=1e-9,
        ):
            errors.append(
                f"{run_prefix}.total_ms: inconsistent with setup, warmup, steady, "
                "and tail intervals"
            )
    ...
    for field in _MEDIAN_FIELDS:
        values = [
            ...
            and isinstance(run.get(field), (int, float))
            and not isinstance(run.get(field), bool)
            and math.isfinite(float(run[field]))
        ]
        recorded = median.get(field)
        if len(values) != len(runs) or not isinstance(recorded, (int, float)):
            continue
        recomputed = statistics.median(values)
        if not math.isclose(float(recorded), recomputed, rel_tol=1e-12, abs_tol=1e-9):
            errors.append(
                f"{prefix}.median.{field}: recorded {recorded!r}, recomputed {recomputed!r}"
            )
    return errors
