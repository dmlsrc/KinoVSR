...
class DispatchPipeline:
    """One Core ML dispatch in flight on a dedicated worker thread.

    The reusable overlap primitive for every ANE processor: submit a
    zero-argument job - a prediction plus its host-side buffer
    bookkeeping, pure Core ML and Python, NEVER MLX, which stays on the
    caller's thread - keep working, and join before touching anything the
    job writes. One slot is deliberate: depth-one pipelining bounds
    memory and latency, keeps dispatches back to back (an ANE dispatch
    issued after 10 ms or more of host idleness pays a 15-23 ms
