...
BSVD_OPTIONS = [
    ...
    Opt(flag='--bsvd-backend',
        ...
        key='backend',
        settings_backed=True,
        help='Which BSVD implementation runs for --denoise bsvd. mlx (default): the reference MLX/GPU path. It stays one continuous recurrent stream under --gop-align: no per-window drain/reset, and noise-map refresh plus pulse history remain continuous (the pulse still reads source sync distance). mpsgraph: direct MPSGraph execution placed on the Neural Engine, with no Core ML package, state, or prediction. Recurrent state and skip rings stay in shared Metal storage; streaming dispatches are pipelined one deep, while --gop-align uses one reusable four-step MPSGraph executable to amortize recurrent-boundary and dispatch overhead without switching runtimes. fp16 only; sides must be multiples of four. The window path covers padded geometries from 128x256 pixels through 1024x576 and fails loudly outside that envelope. ane: the full per-step network as one Core ML dispatch pinned to the Neural Engine, pipelined one dispatch deep so the prediction overlaps the rest of the per-frame work (one extra frame of output delay, absorbed into the family delay). Slightly SLOWER standalone (about 91 vs 78 ms/frame at 640x480 c64) but it vacates the GPU, so chains with other GPU stages can hide its cost; numerics are marginally closer to fp32 than the MLX fp16 path. Under --gop-align through the verified 640x480 padded envelope, fixed eight-step fill 0-7 and drain 8-15 functions omit the None work; the inner boundary steps stay on the full graph to keep only three ANE programs resident. Short light clips can still favor MLX because the extra Core ML functions have a one-time load cost. fp16 only; frames need at least 96 px per side, and widths are reflect-padded internally to the next multiple of 128 (the ANE alignment envelope - CIF 352 runs as 384) and cropped back. Fails loudly rather than fall back: a partial Core ML CPU placement computes this graph WRONG, so non-ANE placement is refused outright. The first run at a new padded geometry builds and compiles the model (seconds to a minute, cached under $KINOVSR_CACHE_DIR); later runs load it in about a second.'),
]
