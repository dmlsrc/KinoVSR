...
BSVD_OPTIONS = [
    ...
    Opt(flag='--bsvd-backend',
        ...
        key='backend',
        settings_backed=True,
        help='Which BSVD implementation runs for --denoise bsvd. mlx (default): the reference MLX/GPU path. ane: the full per-step network as one Core ML dispatch pinned to the Neural Engine, pipelined one dispatch deep so the prediction overlaps the rest of the per-frame work (one extra frame of output delay, absorbed into the family delay). Slightly SLOWER standalone (about 91 vs 78 ms/frame at 640x480 c64) but it vacates the GPU, so chains with other GPU stages can hide its cost; numerics are marginally closer to fp32 than the MLX fp16 path. Note --gop-align widens the gap: its windowing re-processes overlap and drain steps that the MLX path skips nearly for free while the ANE graph pays full price for every step. fp16 only; frames need at least 96 px per side, and widths are reflect-padded internally to the next multiple of 128 (the ANE alignment envelope - CIF 352 runs as 384) and cropped back. Fails loudly rather than fall back: a partial Core ML CPU placement computes this graph WRONG, so non-ANE placement is refused outright. The first run at a new padded geometry builds and compiles the model (seconds to a minute, cached under $KINOVSR_CACHE_DIR); later runs load it in about a second.'),
]
