...
BSVD_OPTIONS = [
    ...
    Opt(flag='--bsvd-backend',
        ...
        key='backend',
        settings_backed=True,
        help='Which BSVD implementation runs for --denoise bsvd. mlx (default): the reference MLX/GPU path. ane: the full per-step network as one Core ML dispatch pinned to the Neural Engine - slightly SLOWER standalone (about 91 vs 78 ms/frame at 640x480 c64) but it vacates the GPU, so chains with other GPU stages can hide its cost; numerics are marginally closer to fp32 than the MLX fp16 path. fp16 only, frames of at least 96 px per side, and it fails loudly rather than fall back (a partial Core ML CPU placement computes this graph WRONG, so the backend refuses non-ANE placement outright). The first run at a new geometry builds and compiles the model (roughly a minute at SD, cached under $KINOVSR_CACHE_DIR); later runs load it in about a second.'),
]
